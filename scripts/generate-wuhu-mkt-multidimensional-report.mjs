...
const OUT_DIR = path.join(ROOT, 'output', 'wuhu-mkt-multidimensional-audience-20260814');
const DATA_PATH = path.join(OUT_DIR, 'wuhu-mkt-multidimensional-analysis.json');
const REPORT_PATH = path.join(OUT_DIR, '三国杀WUHU联盟卡宝98维受众统计深度洞察报告.html');
const METHOD_PATH = path.join(OUT_DIR, '多维受众统计方法与口径.md');
...
<title>三国杀WUHU联盟卡宝98维受众统计深度洞察报告</title>
<style>
...
<main class="page">
  <header class="cover">
    <div>
      <div class="eyebrow">Audience intelligence · Quantitative grounded analysis · 2026-08-14</div>
      <h1>三国杀WUHU联盟卡宝<br>98维受众统计深度洞察</h1>
      <p class="deck">从“评论热度”转向“观测评论者资产”：以 98 个固定指标、34 类统计程序和三国杀玩家语境编码，重建受众结构、行为路径、内容关系与商品表达的可复核证据链。</p>
      <div class="cover-grid">
        <div><strong>${fmt(data.scope.metricDimensions)}</strong><span>固定受众指标</span></div>
        <div><strong>${fmt(data.scope.statisticalMethodCount)}</strong><span>实际统计程序</span></div>
        <div><strong>${fmt(data.coverage.audienceUsers)}</strong><span>观测评论者 U1</span></div>
        <div><strong>${fmt(data.coverage.audienceTextUsers)}</strong><span>有文本评论者 U2</span></div>
      </div>
    </div>
    <div>
      <div class="cover-meta"><span>${fmt(data.coverage.videos)}条视频</span><span>${fmt(data.coverage.capturedComments)}条实采评论</span><span>${fmt(data.coverage.audienceComments)}条观众评论</span><span>采集覆盖 ${pct(data.coverage.capturedComments / data.coverage.declaredComments, 2)}</span><span>${esc(data.coverage.observationStart.slice(0,10))}—${esc(data.coverage.observationEnd.slice(0,10))}</span></div>
      <div class="cover-hint">本报告不把观测评论者外推为全部观看者或粉丝，不把跨视频评论写成平台留存，不把购买语言写成订单转化。</div>
    </div>
  </header>

  <nav class="nav" aria-label="报告章节">
    ${['管理结论','证据母体','受众集中度','生命周期','玩家语境','受众部落','社群机制','商业信号','视频组合','统计证据','MKT系统','90天实验','指标附录'].map((label,index)=>`<a href="#part-${String(index+1).padStart(2,'0')}">${String(index+1).padStart(2,'0')} ${label}</a>`).join('')}
  </nav>
  ...
  <section class="band">
    ...
    <p class="note">用户主键仅在内存中使用评论用户URL聚合；交付的用户级表只保留稳定匿名ID与派生特征，不含昵称、URL、原文和精确评论时间。</p>
  </section>
  ...
  <div class="part-head" id="part-13"><span class="no">13</span><h2>98项指标字典与复算</h2><p>每一个数字都保留方法、分母、解释和边界</p></div>
  <section class="band">
    ...
    <div class="definition-grid">
      ...
      <article><strong>方法与口径</strong><span><a href="多维受众统计方法与口径.md">多维受众统计方法与口径.md</a></span></article>
      <article><strong>生成脚本</strong><span>scripts/analyze-wuhu-mkt-multidimensional.py<br>scripts/generate-wuhu-mkt-multidimensional-report.mjs</span></article>
    </div>
  </section>

  <footer class="footer"><span>三国杀WUHU联盟卡宝 · 98维受众统计深度洞察</span><span>新默认交付 · 生成于 ${esc(data.generatedAt)}</span></footer>
</main>
<script>
...
采集覆盖率为 ${pct(data.coverage.capturedComments/data.coverage.declaredComments,3)}。用户键使用评论用户URL优先，仅在内存聚合；导出用户级表为稳定匿名ID，不含昵称、URL、原文和精确评论时间。
